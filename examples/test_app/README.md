...
Including a value for `var.applications` changes how the workspace is named, tagged, and how AWS authentication variables are configured. This mode is intended for use with a project's "applications".
...
Permissions for the IAM roles created for the workspaces created for each entry in `var.applications{}` are predefined based on the value passed to `var.applications{}.app_type`.
...
Omitting a value for `var.applications` changes how the workspace is named, tagged, and how AWS authentication variables are configured. This variation is intended for use on the project level. More specifically, for creating workspaces which will themselves be used to create _application_ type workspaces.
...
If `workspace_policy` is set for infrastructure workspaces, this module configures OIDC-based authentication for Terraform Cloud and creates a workspace IAM role.

If `workspace_policy` is omitted, an existing AWS Secret called `TerraformSecrets` is expected to be present, with an AWS API credential set in JSON:
...
These values are then populated into workspace variables for AWS authentication.
